...
public class GmdbControllerIntTest {
	...
	public void testGetMovieTitles_ReturnsEmptyList() throws Exception {
		mockMvc.perform(get("/api/movies")).andExpect(status().isOk())
				.andExpect(jsonPath("$.data.movieTitles").isEmpty()).andExpect(jsonPath("$.errorMessages").isEmpty());
	}
	...
	public void testGetMovieDetails_ReturnsMovieNotFound() throws Exception {
		mockMvc.perform(get("/api/movies/{title}", "Dummy")).andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.data").isEmpty()).andExpect(jsonPath("$.errorMessages.length()").value(1))
	...
	@Test
	public void testAddReview_withoutRating() throws Exception {		
		Movie expectedMovieContent = TestHelper.movieContent();
		repository.save(expectedMovieContent);

		mockMvc.perform(post("/api/movies/{title}/review", "Avatar").content(TestHelper.invalidReviewContent())
				.contentType(MediaType.APPLICATION_JSON)).andDo(print())
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.data").isEmpty()).andExpect(jsonPath("$.errorMessages.length()").value(1))
				.andExpect(jsonPath("$.errorMessages[0]")
						.value("Rating is required to submit review. Value can be any number between 0 & 5"));
	}
	
	@Test
	public void testAddReview_withInvalidRating() throws Exception {		
		Movie expectedMovieContent = TestHelper.movieContent();
		repository.save(expectedMovieContent);

		mockMvc.perform(post("/api/movies/{title}/review", "Avatar").content(TestHelper.invalidReviewContent2())
				.contentType(MediaType.APPLICATION_JSON)).andDo(print())
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.data").isEmpty()).andExpect(jsonPath("$.errorMessages.length()").value(1))
				.andExpect(jsonPath("$.errorMessages[0]")
